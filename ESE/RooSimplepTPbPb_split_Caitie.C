...
void RooSplit_ese1D(double centl, double centr, int ese, int opt = 0, int R = 2, TString cFiles2="files14.txt")
{
#ifdef __CINT__
  ...
  hcovariance=new TH2F("covariance","covariance",10,0.,1.,10,0,1.);
   ...
   float ptJet,ptPar, ptdet, centrality, matchradius, eta, area, bkgd, qvec;
...
cout <<"b\n";
    ...
    // get the mc tree
    TTree *mc=(TTree*)input2->Get("JetTree_AliAnalysisTaskJetExtractor_hybJet_AKTChargedR020_tracks_pT0150_pt_scheme_Rho_hybJet"); 
    ...
    mc->SetBranchAddress("Event_Q2Vector", &qvec);
  ...
  int countm=0;
    for(int iEntry=0; iEntry< nEv; iEntry++){
      ...
      if (abs(eta) > (0.9 - (double)R*0.1))   continue;
      posteta++;
      if(ptPar > 200.) continue;    
      if (centrality < centl || centrality > centr)  continue;
      if (ese == 0)  {if (qvec > 2.45) continue;} //20th percentile
      //if (ese == 0)  {if (qvec > 3.55) continue;} //40th percentile
      //if (ese == 1)  {if (qvec < 4.65) continue;} //60th percentile
      if (ese == 1)  {if (qvec < 5.85) continue;} //80th percentile
   ...
   manset.resize(0);
    ...
    stringstream ss;
    if (ese == 0)  ss << "UnfoldingSplit1D_non_R02_" << Form("%.0f", centl) << Form("%.0f", centr) << "_loq2_Jan21.root"; 
    if (ese == 1)  ss << "UnfoldingSplit1D_non_R02_" << Form("%.0f", centl) << Form("%.0f", centr) << "_hiq2_Jan21.root"; 
