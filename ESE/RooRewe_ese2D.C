...
void RooRewe_ese2D(double centl, double centr, int ese, int R = 2, int epcut = 2, TString cFiles2="files15.txt")
{
#ifdef __CINT__
  ...
  int countm=0;
    for(int iEntry=0; iEntry< nEv; iEntry++){
      ...
      scalefactor*=EBscale;
      double weight = 1.0;
      if (ese == 0)  weight = 5.11205*pow(10,-1) + ptPar*(1.63062*pow(10,-2)) + ptPar*ptPar*(-9.73078*pow(10,-5));     //lowq2
      if (ese == 1)  weight = 5.25385*pow(10,-1) + ptPar*(1.46018*pow(10,-2)) + ptPar*ptPar*(-6.42602*pow(10,-5));     //highq2
      //if (ese == 2)  weight = 5.62022*pow(10,-1) + ptPar*(1.38226*pow(10,-2)) + ptPar*ptPar*(-6.79155*pow(10,-5));     //inclusive, out of date
      scalefactor*=weight;
      // *********
  ...
  //==================================================================================
    ...
    stringstream ww;
    if (ese == 0)  ww << "UnfoldingData_2D_non_R02_" << Form("%.0f", centl) << Form("%.0f", centr) << "_lo30_rewe_Feb1.root"; 
    if (ese == 1)  ww << "UnfoldingData_2D_non_R02_" << Form("%.0f", centl) << Form("%.0f", centr) << "_hi30_rewe_Feb1.root"; 
    if (ese == 2)  ww << "UnfoldingData_2D_non_R02_" << Form("%.0f", centl) << Form("%.0f", centr) << "_allq_rewe_Feb1.root"; 
