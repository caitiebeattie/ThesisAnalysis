...
void RooSplit_ese2D(double centl, double centr, int ese, int R = 2, int epcut = 2, TString cFiles2="files15.txt")
{
#ifdef __CINT__
  ...
  std::vector<double> kBinsMeasured = {truncation, 30.0, 40.0, 50.0, 60.0, 70.0, 85.0, 100.0, 120.0};   //semi-central
  ...
  TH2D *h2rawresponse = new TH2D("rawresponse2", "rawresponse2", kBinsMeasured.size()-1, kBinsMeasured.data(), kBinsEPangle.size()-1, kBinsEPangle.data());
  ...
  TH2D* h2rawdata = new TH2D("h2rawdata", "h2rawdata", kBinsMeasured.size()-1, kBinsMeasured.data(), kBinsEPangle.size()-1, kBinsEPangle.data());
  ...
  response2D.Setup(h2rawresponse, h2true);
  ...
  int countm=0;
    for(int iEntry=0; iEntry< nEv; iEntry++){
	  ...
	  h2rawresponse->Fill(ptJet, abs(cos(epangle)), scalefactor);
	  ...
	  response2D.Fill(ptJet, abs(cos(epangle)), ptPar, abs(cos(epPar)), scalefactor);
          ...
          h2rawdata->Fill(ptJet, abs(cos(epangle)), scalefactor);
    ...
    h2rawresponse->Scale(avgevents);
    h1rawdata->Scale(avgevents);
    h2rawdata->Scale(avgevents);
    h1true->Scale(avgevents);
    h2true->Scale(avgevents);
    //////////efficiencies done////////////////////////////////////
  ...
  //==================================================================================
    ...
    stringstream ww;
    if (ese == 0)  ww << "UnfoldingSplit_2D_non_R02_" << Form("%.0f", centl) << Form("%.0f", centr) << "_lo30_Feb7.root"; 
    if (ese == 1)  ww << "UnfoldingSplit_2D_non_R02_" << Form("%.0f", centl) << Form("%.0f", centr) << "_hi30_Feb7.root"; 
    if (ese == 2)  ww << "UnfoldingSplit_2D_non_R02_" << Form("%.0f", centl) << Form("%.0f", centr) << "_30q_Feb7.root"; 
    TFile *fout=new TFile (ww.str().c_str(),"RECREATE");
    fout->cd();
    h1rawresponse->SetName("rawresponse1");
    h1rawresponse->Write();
    h2rawresponse->SetName("rawresponse2");
    h2rawresponse->Write();
    h1rawdata->SetName("rawdata1");
    h1rawdata->Write();
    h2rawdata->SetName("rawdata2");
    h2rawdata->Write();
  ...
  //==================================================================================
    ...
    for(int jar = 1; jar < 16; jar++) {
      ...
      RooUnfoldBayes unfold(&response2D, h2rawdata, iter, false);    // OR
